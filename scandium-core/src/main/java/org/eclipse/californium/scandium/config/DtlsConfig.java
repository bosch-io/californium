...
public final class DtlsConfig {
	...
	/**
	 * List of signature and hash algorithm values definition.
	 */
	...
	public static final TimeDefinition DTLS_SESSION_TIMEOUT = new TimeDefinition(MODULE + "SESSION_TIMEOUT",
			"DTLS session timeout.", 1L, TimeUnit.HOURS);
	/**
	...
	/**
	 * Specify the certificate authentication mode of the other peer.
	 * 
	 * Used on the server-side to request a client certificate for
	 * authentication.
	 */
	public static final EnumDefinition<CertificateAuthenticationMode> DTLS_CERTIFICATE_AUTH_MODE = new EnumDefinition<>(
			MODULE + "CERTIFICATE_AUTH_MODE", "DTLS certificate authentication mode of the other peer.",
	...
	/**
	 * Use anti replay filter for records in window and before limit.
	 * 
	 ...
	 *      target= "_blank">RFC6347 4.1.2.6. Anti-Replay</a>
	 */
	public static final IntegerDefinition DTLS_USE_EXTENDED_WINDOW_ANTI_REPLAY_FILTER = new IntegerDefinition(
			MODULE + "USE_EXTENDED_WINDOW_ANTI_REPLAY_FILTER",
			"DTLS use a extended window for the anti-replay-filter. -1 := extend window to start of session, 0 := normal window, <n> := extend window by <n>.",
			0);
	/**
	...
	static {
		...
		Configuration.addModule(MODULE, new DefinitionsProvider() {
			...
			public void applyDefinitions(Configuration config) {
				...
				config.set(DTLS_CERTIFICATE_AUTH_MODE, CertificateAuthenticationMode.NEEDED);
				...
				config.set(DTLS_USE_EXTENDED_WINDOW_ANTI_REPLAY_FILTER, 0);
